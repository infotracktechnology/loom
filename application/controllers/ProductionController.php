...
class ProductionController extends CI_Controller {
    ...
    public function loom($id){
        $loom = $this->db->query("SELECT a.loom_id,c.id,c.size,c.pick,c.product_name,((1848*d.count*36)/(39.36*c.pick*52+2))grm,c.coolie FROM `loom_master` a left join loom_product b on a.loom_id=b.loom_id left join product_master c on b.product_id=c.id left join material_master d on c.weft_yarn=d.material_id WHERE a.loom_id=$id")->row();
